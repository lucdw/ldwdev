...
# "ldwdev(function):" and formatted to have a maximum line length of 'txt_width'
...
dev_msg <- function(wat, txt_width = getOption("width", 80L), indent = 2L) {
  x <- sub("[() ].*$", "", as.character(sys.calls()))
  ignore_in_stack <- c(
    "^eval$", "^try", "^doTryCatch", "^dev_stop", "^dev_warn",
     "^warning$", "^stop$", "^dev_note",
    "^dev_cat", "^dev_msg", "^which$", "^unique$", "^as\\.",
    "^unlist$", "^message$", "^source$",
    "^withVisible$", "^tryCatch.W.E$", "^withCallingHandlers$", "^do.call$"
  )
  ignores <- rep(FALSE, length(x))
  for (ptrn in ignore_in_stack) {
 ...
 chunk_size <- nchar(chunks)
  ...
  while (nstart <= length(chunks)) {
    while (
      nstop < length(chunks) &&
        sum(chunk_size[seq.int(nstart, nstop + 1L)]) +
          nstop -
          nstart +
          indent <
          txt_width
